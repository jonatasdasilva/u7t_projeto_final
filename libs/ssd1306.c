...
void ssd1306_line(ssd1306_t *ssd, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, bool value) {
    ...
    int err = dx - dy; // Calcula a distância absoluta entre y1 e y0 (diferença vertical), mas com sinal negativo.
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
  ...
  // Verifica o tipo de caractere e calcula o índice correto na matriz font[]
  if (c >= 'A' && c <= 'Z') {
    // Letras maiúsculas (A-Z)
    index = (c - 'A' + 11) * 8; // 11 é o deslocamento para letras maiúsculas
  } else if (c >= 'a' && c <= 'z') {
    // Letras minúsculas (a-z)
    index = (c - 'a' + 37) * 8; // 37 é o deslocamento para letras minúsculas
  } else if (c >= '0' && c <= '9') {
    // Números (0-9)
    index = (c - '0' + 1) * 8; // 1 é o deslocamento para números
  } else {
    // Caracteres especiais
    switch (c) {
      case '!': index = 0; break; // Índice do caractere '!'
      case '"': index = 8; break; // Índice do caractere '"'
      case '#': index = 16; break; // Índice do caractere '#'
      case '$': index = 24; break; // Índice do caractere '$'
      case '%': index = 32; break; // Índice do caractere '%'
      case '&': index = 40; break; // Índice do caractere '&'
      case '\'': index = 48; break; // Índice do caractere '''
      case '(': index = 56; break; // Índice do caractere '('
      case ')': index = 64; break; // Índice do caractere ')'
      case '*': index = 72; break; // Índice do caractere '*'
      case '+': index = 80; break; // Índice do caractere '+'
      case ',': index = 88; break; // Índice do caractere ','
      case '-': index = 96; break; // Índice do caractere '-'
      case '.': index = 104; break; // Índice do caractere '.'
      case '/': index = 112; break; // Índice do caractere '/'
...
/* desenha um gráfico de onda no display SSD1306 a partir das amostras da onda a serem desenhadas */
void draw_waveform(ssd1306_t *display, uint8_t *waveform) {
    ssd1306_fill(display, false); // Limpa o display
    ssd1306_rect(display, 0, 0, 128, 64, true, false); // Desenha um retângulo
    ssd1306_draw_string(display, "Leitura!", 38, 2); // Desenha uma string

    // Desenha o gráfico de onda baseado nas amostras coletadas
    for (int i = 1; i < WIDTH; i++) {
        ssd1306_line(display, i , waveform[i - 1], (i - 1), waveform[i], true);
    }
